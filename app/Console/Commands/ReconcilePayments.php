...
class ReconcilePayments extends Command
{
    ...
    {
        app(\App\Services\Payments\S3pCallbackProcessor::class)->processDue($mobile);
        if (config('payments.s3p.enabled')) {
            Subscription::whereNotNull('s3p_pay_item')->where(function ($q) {
                ...
                try { $mobile->refresh($sub); }
                catch (\Throwable $e) { PaymentAudit::record($sub->id, 'mobile_reconciliation_failed', ['exception' => $e::class]); }
            });
        }
        Subscription::where('valuation_status', 'awaiting_nav')->chunkById(100, function ($rows) use ($bank) {
            foreach ($rows as $sub) {
                try { $bank->value($sub); }
                catch (\Throwable $e) { PaymentAudit::record($sub->id, 'valuation_retry_required', ['exception' => $e::class]); }
            }
        });
