...
class S3pGateway
{
    ...
    {
        if (!in_array($operator, ['orange_money', 'mtn_momo'], true)) return false;
        if (app()->environment('production') && $this->isStaging()) return false;
        if (config('payments.s3p.simulation') && !$this->isSimulation()) return false;
    ...
    {
        if (config('payments.s3p.simulation')) throw new RuntimeException('Simulation réservée aux tests isolés.');
        if (!config('payments.s3p.enabled')) throw new RuntimeException('S3P désactivé.');
        $base = $this->base();
        $ca = config('payments.s3p.ca_bundle') ?: true;
        if ($ca !== true && (!is_string($ca) || !is_file($ca) || !is_readable($ca))) throw new RuntimeException('Autorités TLS indisponibles.');
        $cacheKey = 's3p.token.'.hash('sha256', $base.'|'.config('payments.s3p.public_key').'|'.config('payments.s3p.secret_key'));
        if ($freshToken) Cache::forget($cacheKey);
        ...
        if (!$token) {
            ...
            if (!is_string($token) || $token === '' || preg_match('/\s/', $token)
                ...
                || (isset($data['token_type']) && strcasecmp($data['token_type'], 'Bearer') !== 0)) throw new RuntimeException('Jeton S3P invalide.');
    ...
    public function quote(Subscription $sub): array
    {
        ...
        if (!$this->available($sub->mobile_provider)) throw new RuntimeException('Opérateur non configuré.');
        $catalog = $this->read('cashout', ['serviceid' => $service, 'merchant' => $merchant]);
        if (!is_array($catalog) || !array_is_list($catalog)) throw new RuntimeException('Catalogue S3P invalide.');
        ...
        if (!is_string($item) || strlen($item) > 255 || (string) ($catalog[0]['serviceid'] ?? '') !== $service
            ...
            || ($catalog[0]['amountType'] ?? null) !== 'CUSTOM') throw new RuntimeException('Produit d’encaissement incompatible.');
        ...
        if (!is_array($quote) || !$this->validId($quote['quoteId'] ?? null) || ($quote['payItemId'] ?? null) !== $item
            ...
            || (isset($quote['amountLocalCur']) && !$this->sameMoney($quote['amountLocalCur'], $sub->montant_total))) throw new RuntimeException('Le devis S3P ne correspond pas au total confirmé.');
        ...
        if (!$expiry || $expiry->lte(now()->addSeconds(5))) throw new RuntimeException('Devis expiré ou expiration absente.');
        $format = config('payments.s3p.wallet_formats.'.$sub->mobile_provider);
        if (!in_array($format, ['national', 'international'], true)) throw new RuntimeException('Format de portefeuille non configuré.');
    ...
    public function collect(Subscription $sub): array
    {
        ...
        if (!$sub->s3p_quote_expires_at || $sub->s3p_quote_expires_at->lte(now()->addSeconds(2))) throw new RuntimeException('Devis expiré avant exécution.');
        ...
        if (!is_array($data) || !$this->validId($data['ptn'] ?? null)
            ...
            || (isset($data['priceLocalCur']) && !$this->matchesAmount($sub, $data))) throw new RuntimeException('Réponse d’encaissement incohérente. Vérification obligatoire.');
        return $data;
    }
    ...
    public function verify(Subscription $sub): array
    {
        ...
        if ($simulated !== $this->isSimulation()) throw new RuntimeException('Transaction et environnement de paiement incompatibles.');
        if ($this->isSimulation()) {
            return [
        ...
        if (is_array($data) && array_is_list($data)) {
            if (count($data) !== 1) throw new RuntimeException('Résultat S3P ambigu.');
            $data = $data[0];
        }
        ...
        if (!in_array($status, ['PENDING', 'SUCCESS', 'ERRORED', 'REVERSED'], true)
            ...
            || ($error !== null && $status === 'REVERSED' && (int) $error !== 3)) throw new RuntimeException('Statut et code fournisseur incohérents.');
        foreach (['receiptNumber', 'veriCode'] as $field) {
            ...
            if (isset($data[$field]) && (!is_string($data[$field]) || mb_strlen($data[$field]) > 100
                || preg_match('/[\x00-\x1F\x7F]/', $data[$field]))) throw new RuntimeException('Détail de transaction invalide.');
        }
        if (isset($data['timestamp']) && (!is_string($data['timestamp']) || strlen($data['timestamp']) > 64)) throw new RuntimeException('Horodatage fournisseur invalide.');
        return $data;
    }
    ...
    private function sameMoney(mixed $amount, mixed $expected): bool
    {
        if ((!is_string($amount) && !is_int($amount) && !is_float($amount))
            || !preg_match('/^\d{1,12}(?:\.\d{1,8})?$/D', (string) $amount)) return false;
        return \Brick\Math\BigDecimal::of((string) $amount)->isEqualTo(\Brick\Math\BigDecimal::of((string) $expected));
    }
    ...
    private function assertContext(Subscription $sub): void
    {
        ...
        if (!is_array($context) || ($context['base_url'] ?? null) !== $this->base()
            ...
            || !$this->sameMoney($context['amount'] ?? null, $sub->montant_total)) throw new RuntimeException('Contexte de transaction absent ou modifié.');
    }

    private function walletNumber(Subscription $sub): string
    {
        $number = (string) $sub->payment_phone;
        if (!preg_match('/^2376\d{8}$/D', $number)) throw new RuntimeException('Numéro de portefeuille invalide.');
