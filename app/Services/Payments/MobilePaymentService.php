...
class MobilePaymentService
{
    ...
    public function start(Subscription $subscription): Subscription
    {
        ...
        $claimed = DB::transaction(function () use ($subscription) {
            ...
            if (($sub->s3p_reference && $sub->mobile_state !== 'quote_failed') || $sub->statut === 'Succès') return false;
        ...
        try {
            ...
            if (empty($result['ptn'])) throw new \RuntimeException('PTN absent.');
            DB::transaction(function () use ($sub, $result) {
                $locked = Subscription::lockForUpdate()->findOrFail($sub->id);
                if ($locked->s3p_ptn && $locked->s3p_ptn !== (string) $result['ptn']) throw new \RuntimeException('PTN incohérent.');
    ...
    public function refresh(Subscription $subscription, bool $requireFresh = false, ?object $callback = null): Subscription
    {
        $eligible = DB::transaction(function () use ($subscription) {
            $sub = Subscription::lockForUpdate()->findOrFail($subscription->id);
            if (!$sub->s3p_reference || !$sub->s3p_pay_item || (!$this->gateway->isSimulation() && $sub->mobile_checked_at && $sub->mobile_checked_at->gt(now()->subSeconds(10)))) return false;
            $sub->forceFill(['mobile_checked_at' => now()])->save();
            return true;
        });
        if (!$eligible) {
            if ($requireFresh) throw new \RuntimeException('Vérification différée par la cadence fournisseur.');
        ...
        return DB::transaction(function () use ($subscription, $state, $callback) {
            ...
            if ($sub->mobile_state === 'reversed') return $sub;
            if (!in_array($status, ['PENDING', 'SUCCESS', 'ERRORED', 'REVERSED'], true)) throw new \RuntimeException('Statut S3P inconnu.');
            if ($sub->s3p_ptn && $sub->s3p_ptn !== (string) $state['ptn']) throw new \RuntimeException('PTN différent.');
            if ($sub->statut === 'Succès' && $status !== 'REVERSED') return $sub;
            // Terminal states cannot be overwritten by older concurrent responses.
            if ($sub->mobile_state === 'success' && !in_array($status, ['SUCCESS', 'REVERSED'], true)) return $sub;
            ...
            if ($status === 'SUCCESS' && $sub->statut !== 'Succès') {
                if (S3pGateway::mustKeepTestFundsSeparate($sub)) {
                    ...
                    if ($timestamp === null && !$callback && !config('payments.s3p.verified_timestamp_is_receipt') && !$this->gateway->isSimulation()) {
                        ...
                        if ($received->isFuture() || $received->lt($sub->created_at->copy()->startOfSecond())) throw new \RuntimeException('Date fournisseur incohérente.');
            ...
            if ($sub->isDirty()) {
                ...
                if ($status === 'REVERSED') {
                    \App\Models\Notification::create(['user_id' => $sub->user_id, 'title' => 'Paiement à vérifier',
                        'body' => 'Le prestataire a signalé une annulation du paiement '.$sub->reference_transaction.'. Contactez le support.', 'type' => 'danger']);
                }
            }
            if ($status === 'SUCCESS') return app(BankPaymentService::class)->value($sub);
            return $sub->fresh();
        });
    }
}
